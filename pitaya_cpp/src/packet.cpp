...
std::vector<uint8_t> Codec::Encode(PacketType type,
                                   const std::vector<uint8_t>& data) {
    uint8_t t = static_cast<uint8_t>(type);
    if (t < static_cast<uint8_t>(PacketType::Handshake) ||
        t > static_cast<uint8_t>(PacketType::Kick)) {
...
std::vector<Packet> Codec::Decode(const std::vector<uint8_t>& data,
                                  size_t& consumed) {
