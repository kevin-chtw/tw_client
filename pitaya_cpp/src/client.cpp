...
namespace pitaya
{
    ...
    bool Client::Notify(const std::string &route, const google::protobuf::Message &msg)
    {
        std::string data = msg.SerializeAsString();
        return Notify(route, std::vector<uint8_t>(data.begin(), data.end()));
    }
    ...
    std::future<Message> Client::Request(const std::string &route, const google::protobuf::Message &msg)
    {
        std::string data = msg.SerializeAsString();
        return Request(route, std::vector<uint8_t>(data.begin(), data.end()));
    }
